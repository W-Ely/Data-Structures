...
def bst_left_balance():
    """Create a binary search tree 5 numbers snake.
...
@pytest.fixture
def bst_balanced():
    """Create a binary search tree 5 numbers.
...
def bst_100_rand():
    ...
    tree = Bst([randint(1, 1000) for _ in range(100)])
    return tree
...
def test_search_finds_node(bst_balanced):
    ...
    assert bst_balanced.search(1).value == 1
...
def test_contains_returns_true_on_tree_with_value(bst_balance):
    """Tree has value true."""
    assert bst_balance.contains(6) is True
    assert bst_balance.contains(3) is True
